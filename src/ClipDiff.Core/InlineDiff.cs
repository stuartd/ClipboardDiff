...
internal sealed class InlineDiff(bool ignoreSpacing, CancellationToken cancellationToken)
{
    ...
    internal DiffRow Highlight(DiffRow row)
    {
        ...
        if ((long)row.OldText.Length + row.NewText.Length > MaximumPairBytes)
            return row;
        var bytes = Encoding.UTF8.GetByteCount(row.OldText) + Encoding.UTF8.GetByteCount(row.NewText);
        if (bytes > MaximumPairBytes || bytes > _remainingWork)
            return row;
        _remainingWork -= bytes;
        var searchBudget = Math.Min(MaximumSearchCells, _remainingWork);
        var initialBudget = searchBudget;
        ...
        var oldStart = 0;
        var newStart = 0;
        foreach (var (oldIndex, newIndex) in tokenMatches.Append((oldTokens.Count, newTokens.Count)))
        {
            cancellationToken.ThrowIfCancellationRequested();
            var oldEnd = oldIndex == oldTokens.Count ? oldUnits.Count : oldTokens[oldIndex].Start;
            var newEnd = newIndex == newTokens.Count ? newUnits.Count : newTokens[newIndex].Start;
            var matches = Align(oldUnits.GetRange(oldStart, oldEnd - oldStart),
                newUnits.GetRange(newStart, newEnd - newStart), ref searchBudget, weighted: false);
            foreach (var (oldUnit, newUnit) in matches)
            {
    ...
    internal static string NormalizeSpacing(string text) =>
        string.Concat(Units(text, true).Select(unit => unit.Text));
    ...
    private static List<Unit> Units(string text, bool normalize)
    {
        ...
        for (var index = 0; index < boundaries.Length; index++)
        {
            ...
            var value = text[boundaries[index]..end];
            if (normalize && IsWhitespace(value))
            {
                if (units.Count > 0)
                {
                    if (units[^1].Text == " ")
                        ...
                        units.Add(new Unit(" ", index, index + 1, 1));
                }
            }
            else
                units.Add(new Unit(value, index, index + 1, 1));
        }
    ...
    private static bool IsWhitespace(string text) => text.All(char.IsWhiteSpace);
    private static int Category(string text) => IsWhitespace(text) ? 0 :
    ...
    private static List<Unit> Tokens(List<Unit> units)
    {
        ...
        {
            var category = Category(units[start].Text);
            var end = start + 1;
            while (category != 2 && end < units.Count && Category(units[end].Text) == category)
                end++;
            tokens.Add(new Unit(string.Concat(units.GetRange(start, end - start).Select(unit => unit.Text)),
    ...
    private static IReadOnlyList<HighlightRange> ChangedRanges(List<Unit> units, bool[] matched)
    {
        ...
        for (var index = 0; index < units.Count; index++)
        {
            ...
            if (ranges.Count > 0 && ranges[^1].End == unit.Start)
                ...
                ranges.Add(new HighlightRange(unit.Start, unit.End));
        }
